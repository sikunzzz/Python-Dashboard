...
def load_TRTickdata(filenames, ticker_list):
    ...
    for i in range(len(filenames)):
        ...
        if os.path.exists(filenames[i]):
             dataframe_dict[ticker_list[i]] = pd.read_json(filenames[i])
            ...
            query = """ SELECT
                        ...
                        AND
                        RIC=""" + "'" + ticker_list[i] + "'" +" ORDER BY Date_Time"
            ...
            tickjString = df.to_json()
            with open(filenames[i], "+w") as f:
                f.write(tickjString)

                dataframe_dict[ticker_list[i]] = df
